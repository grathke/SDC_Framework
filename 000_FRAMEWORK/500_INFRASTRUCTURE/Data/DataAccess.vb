...
Namespace SDC.Framework
    Public NotInheritable Class DataAccess
        ...
        ''' <summary>
        ''' The generation request behind a page, found by the page's own name, or 0 for a page
        ''' nobody generated.
        '''
        ''' FW_PageSettings_U is opened with a page name rather than a request id - so it can serve
        ''' a hand-written page too - and needs the request only for Apply Fields. Matches either
        ''' half of the pair, since the caller may hold whichever name it was opened with.
        ''' </summary>
        ''' <summary>
        ''' Renames a page, for FW_PageSettings_U.
        '''
        ''' Table_Alias is the default in the caption chain, so this reaches the page's own title,
        ''' the button that opens it and any list that names pages - one value rather than three
        ''' that can disagree.
        '''
        ''' Updates a page that exists and never creates one, and patches the cache on success:
        ''' the alias is read from the cached FW_Pages row, so a rename that only reached the
        ''' database would appear to have done nothing until the application restarted.
        ''' </summary>
        Public Shared Function SavePageCaption(windowOrPageName As String, caption As String, updatedBy As Integer) As Boolean
            If String.IsNullOrWhiteSpace(windowOrPageName) OrElse String.IsNullOrWhiteSpace(caption) Then Return False

            Dim trimmedName = windowOrPageName.Trim()
            Dim trimmedCaption = caption.Trim()
            ...
            Try
                Using conn As New SqlConnection(ConnectionString)
                    ...
                    Using cmd As New SqlCommand(
                        "UPDATE dbo." & PagesTable & " " &
                        "SET Table_Alias = @Alias, ModifiedBy = @ModifiedBy, ModifiedOn = GETDATE() " &
                        "WHERE WindowOrPage = @WindowOrPage", conn)

                        cmd.Parameters.AddWithValue("@Alias", trimmedCaption)
                        cmd.Parameters.AddWithValue("@ModifiedBy", updatedBy)
                        cmd.Parameters.AddWithValue("@WindowOrPage", trimmedName)
            ...
            SyncLock metadataCacheLock
                If pageAliasCacheLoaded AndAlso pageAliasCache.ContainsKey(trimmedName) Then
                    pageAliasCache(trimmedName) = trimmedCaption
                End If
            End SyncLock
        ...
        Public Shared Function GetPageGenerationIdForPage(pageName As String) As Integer
            If String.IsNullOrWhiteSpace(pageName) Then Return 0

            Using conn As New SqlConnection(ConnectionString)
                conn.Open()
                Using cmd As New SqlCommand(
                    "SELECT TOP 1 GeneratedPageID FROM dbo." & GeneratedPagesTable & " " &
                    "WHERE (BrowsePageName = @PageName OR MaintenancePageName = @PageName) " &
                    "AND ISNULL(DeletedFlag, 0) = 0 " &
                    "ORDER BY GeneratedPageID DESC", conn)

                    cmd.Parameters.AddWithValue("@PageName", pageName.Trim())
                    Dim result = cmd.ExecuteScalar()
                    Return If(result Is Nothing OrElse IsDBNull(result), 0, Convert.ToInt32(result, CultureInfo.InvariantCulture))
                End Using
            End Using
        End Function
