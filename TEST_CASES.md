...
| MENU-31 | Two more tiles fit at the minimum window | At the 1180 minimum, the movable row holds 8 tiles against the 6 in use. `MovableTileCapacityAtMinimumWidth` reports that figure rather than the capacity at the current width, because a tile that fits only when the window is wide is clipped silently when it is not — the panel neither wraps nor scrolls. | pass 2026-09-04 |
