...
| GEN-18 | Ticking Lookup produces a usable lookup | **Defect, found 2026-09-01.** Ticking Lookup in Select Fields stores only the field name (`JoinCheckedGridFields`), but the generator requires `<Field> -> <Table>.<Value> displayed as <Display>` and refuses anything else. The Lookup Fields box is read-only, so the full form cannot be typed either. A lookup can be ticked but never generated, which is why GEN-03 and GEN-04 have never been exercised. | fail 2026-09-01 |
