...
| MENU-02 | Reorder a tile | Drag `Entity` ahead of `Users`. The row reflows as the pointer passes the neighbour's midpoint, and the order survives closing and reopening the menu. | pass 2026-09-03 |
...
| MENU-23 | A click elsewhere closes the menu | Open the menu, then click a different ribbon tile. The menu closes and the other tile does its own job. Clicking `Menu Test` afterwards opens it normally. | untested |
