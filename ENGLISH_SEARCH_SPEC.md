...
A pair, added to `000_FRAMEWORK\000_BASE CLASSES\Base_B.vb` **once**, so every browse page inherits them and there is no
...
| `000_FRAMEWORK\000_BASE CLASSES\Base_B.vb` | the text box, the button, the wire-up | **`Create Base_B Restore Point` before the first substantive edit** — QBE Visibility Guardrail |
...
- **An echo of what was parsed.** Populating the grid would have shown the user what was understood;
  ...
  `UpdateActiveFilterLabel` maintains (`000_FRAMEWORK\000_BASE CLASSES\Base_B.vb:4103`).
