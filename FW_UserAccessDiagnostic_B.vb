...
Namespace SDC.Framework
    Public Class FW_UserAccessDiagnostic_B
        ...
        Protected Overrides Sub ApplyPageSpecificLayout()
            ...
            browseSplit.Top = 128
            browseSplit.Height = 410
            ApplyDiagnosticQbeLayout(browseSplit)
            ...
            applyChangesButton.Left = checkAccessButton.Right + 8
            applyChangesButton.Top = checkAccessButton.Top
            permissionsGrid.Left = browseSplit.Left
            permissionsGrid.Top = tableComboBox.Bottom + 8
            ...
            analysisResultLabel.Top = permissionsGrid.Bottom + 8
            ...
            diagnosticResultTextBox.Height = 130
        ...
        Private Sub CheckAccessButton_Click(sender As Object, e As EventArgs)
            ...
            Dim selectedIssues = New List(Of String)(New String() {
                "Read records",
                "Create records",
                "Modify records",
                "Delete records",
            ...
            For Each issue In selectedIssues
                Dim allowed As Boolean
                Dim reason As String
                Select Case issue
                    Case "Read records"
                        allowed = capabilities("Can_Read")
                        reason = "Can_Read = " & allowed.ToString()
                    Case "Create records"
                        allowed = capabilities("Can_Create")
                        reason = "Can_Create = " & allowed.ToString()
                    Case "Modify records"
                        allowed = capabilities("Can_Update")
                        reason = "Can_Update = " & allowed.ToString()
                    Case "Delete records"
                        allowed = capabilities("Can_Delete")
                        reason = "Can_Delete = " & allowed.ToString()
                    Case "Use QBE"
                        allowed = capabilities("Can_UseQBE")
                        reason = "Can_UseQBE = " & allowed.ToString()
                    Case "View all records"
                        allowed = capabilities("Can_ViewAllRecords")
                        reason = "Can_ViewAllRecords = " & allowed.ToString()
                    Case Else
                        allowed = capabilities("Can_ViewOnlyMyRecords")
                        reason = "Can_ViewOnlyMyRecords = " & allowed.ToString()
                End Select

                result.AppendLine(issue & ": " & If(allowed, "granted", "denied") & ". " & reason)
            Next
        ...
        Private Sub ApplyDiagnosticQbeLayout(browseSplit As SplitContainer)
