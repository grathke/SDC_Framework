...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\FW_Base_U.vb" -Pattern 'indicators.MarkAllRequiredTouched()' -Description "Shared _U refreshes local required borders during validation"
...
Assert-Pattern -Path ".\000_FRAMEWORK\500_INFRASTRUCTURE\Widgets\FieldIndicators.vb" -Pattern 'touched.Add(control)' -Description "Shared AddField required borders activate only once a required control is visited"
Assert-Pattern -Path ".\000_FRAMEWORK\500_INFRASTRUCTURE\Widgets\FieldIndicators.vb" -Pattern 'border.BackColor = If(showWarning, WarningColor, SystemColors.Control)' -Description "Shared AddField required borders refresh color with control values"
Assert-Pattern -Path ".\000_FRAMEWORK\500_INFRASTRUCTURE\Widgets\FieldIndicators.vb" -Pattern 'If Not IsEmptyRequired(control) Then Return False' -Description "Shared AddField required borders stay hidden while a required control holds a value"
Assert-Pattern -Path ".\000_FRAMEWORK\500_INFRASTRUCTURE\Widgets\FieldIndicators.vb" -Pattern 'Return touched.Contains(control) OrElse hovered.Contains(control)' -Description "Shared AddField required borders show only once a required control is visited or hovered"
...
Assert-Pattern -Path ".\000_FRAMEWORK\085_MESSAGING\MessageComposeForm.vb" -Pattern "SelectionMode.MultiSimple" -Description "Compose selects several recipients by plain click (no Ctrl or Shift, which Thinfinity does not carry)"
