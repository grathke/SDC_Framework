...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "DeletedViewGuard.ResultHasDeletedFlagColumn(browseGrid)" -Description "Base browse uses shared result-column guard"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "DeletedViewGuard.TableSupportsDeletedView(tableName)" -Description "Base browse uses shared table deleted support guard"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "If col Is Nothing OrElse Not col.Visible Then" -Description "QBE derives fields only from visible browse columns"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "If IsPkAliasColumn(col) OrElse IsSoftDeleteColumnName(fieldName) Then" -Description "Grid-derived QBE excludes the internal PK alias"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern 'String.Equals(dc.ColumnName, "PK", StringComparison.OrdinalIgnoreCase)' -Description "Start Empty QBE excludes the internal PK alias"
if (Select-String -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern 'AssignedManagerID' -SimpleMatch -Quiet) {
...
if (Select-String -Path ".\000_FRAMEWORK\080_HELP DESK\HD_Issues_B.vb" -Pattern "Overrides Function GetActiveBaseSql" -SimpleMatch -Quiet) {
...
if (Select-String -Path ".\000_FRAMEWORK\080_HELP DESK\HD_Issues_Support_B.vb" -Pattern "Inherits FW_HD_Issues_B|New FW_HD_Issues_B" -SimpleMatch -Quiet) {
    throw "FW_HD_Issues_Support_B must be independent of FW_HD_Issues_B."
}
Assert-Pattern -Path ".\000_FRAMEWORK\080_HELP DESK\HD_Issues_Support_B.vb" -Pattern "Inherits FW_Base_B" -Description "Support browse independently inherits shared Base_B"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "Return Me.GetType().Name" -Description "Browse pages use runtime class names for FW_RoleTables keys"
...
Assert-Pattern -Path ".\000_FRAMEWORK\010_MAIN MENU\MainMenu.vb" -Pattern "New Dashboard_Application(currentUser, activeAccessProfile)" -Description "Main menu settings tile passes access context to the application dashboard"
Assert-Pattern -Path ".\000_FRAMEWORK\010_MAIN MENU\MainMenu.vb" -Pattern "New Dashboard_Company(currentUser, activeAccessProfile)" -Description "Main menu settings tile passes access context to the company dashboard"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern '.Size = New Size(120, 26)' -Description "Shared _U captions match the 26-pixel edit-control height"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern 'RefreshLocalRequiredBorders()' -Description "Shared _U refreshes local required borders during validation"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern 'touchedRequiredControls.Add(control)' -Description "Shared AddField required borders activate only once a required control is visited"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern 'border.BackColor = If(showWarning, Color.Red, SystemColors.Control)' -Description "Shared AddField required borders refresh color with control values"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern 'If Not IsEmptyRequiredControl(control) Then Return False' -Description "Shared AddField required borders stay hidden while a required control holds a value"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern 'Return touchedRequiredControls.Contains(control) OrElse hoveredRequiredControls.Contains(control)' -Description "Shared AddField required borders show only once a required control is visited or hovered"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "UpsertPageRecord" -Description "Base browse persists generated fallback SQL"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "EnsureDefaultLayoutExists(registrationId)" -Description "Base browse ensures a shared default layout after the first successful grid load"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "updateButton.PerformClick()" -Description "Base browse double-click invokes the visible Modify button"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "If Not layoutChanged AndAlso Not String.IsNullOrWhiteSpace(existingLastUsed) Then" -Description "Base browse does not rewrite Last Used without a user layout change"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern '"LastUsed",' -Description "Base browse upserts Last Used when the grid JSON changed"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern "WarnIfMissingRowVersion" -Description "Base maintenance page warns on missing RowVersion"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern "CaptureOriginalRowVersion" -Description "Base maintenance page owns the original RowVersion"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern "CopyOriginalRowVersion" -Description "Base maintenance page provides RowVersion copies"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "TableHasRowVersion" -Description "Base browse page checks RowVersion schema"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern "ConfirmConcurrencyOverwrite" -Description "Base maintenance flow requires explicit conflict choice"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern "bypassCancelCloseCheck" -Description "Base maintenance page controls close-after-save behavior"
...
Assert-Block -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Block @"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "SoftDeleteGeneratedPageRecord" -Description "FW_Base_B owns the standard delete"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "RestoreGeneratedPageRecord" -Description "FW_Base_B owns the standard restore"
...
Assert-Pattern -Path ".\000_FRAMEWORK\010_MAIN MENU\MainMenu.vb" -Pattern "SizeType.Percent, 34.0F" -Description "Messages region is widened"
Assert-Pattern -Path ".\000_FRAMEWORK\010_MAIN MENU\MainMenu.vb" -Pattern "SizeType.Percent, 33.0F" -Description "Other main-menu regions share the reduced width"
...
Assert-Pattern -Path ".\000_FRAMEWORK\080_HELP DESK\HD_Issues_U.vb" -Pattern "Conversation History" -Description "Help Desk update page provides conversation history"
...
Assert-NotPattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "SessionState.Current.Value.UserID" -Description "Base browse stamps the acting user, not the session"
Assert-NotPattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern "SessionState.Current.Value.UserID" -Description "Base maintenance stamps the acting user, not the session"
...
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_U.vb" -Pattern "SwitchedUserGuard.AllowWrite" -Description "Saving while switched is refused"
Assert-Pattern -Path ".\000_FRAMEWORK\000_BASE CLASSES\Base_B.vb" -Pattern "SwitchedUserGuard.AllowWrite" -Description "Delete and restore while switched are refused"
